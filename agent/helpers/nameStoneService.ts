...
export class NameStoneService {
  ...
  private initializeClient(): boolean {
    try {
      if (!this.config.apiKey) {
        console.warn("⚠️ NameStone API key not found. Skipping subname creation.");
        return false;
      }
   ...
   */
  async createSubnameForCalendar(calendarId: string): Promise<SubnameCreationResult> {
    try {
      ...
      console.log(`🏷️ Attempting to create subname: ${fullName} for calendar: ${calendarId}`);
      ...
      const response = await this.ns.setName({
        ...
        description: `Calendar wallet for ${calendarId}`,
      });
      ...
      console.error(`❌ Failed to create subname for calendar ${calendarId}:`, error);
   ...
   */
  async createSubnameWithRetry(calendarId: string, maxRetries: number = 3): Promise<SubnameCreationResult> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (result.error?.includes("Authentication") || result.error?.includes("Network")) {
          console.warn(`⚠️ Non-retryable error on attempt ${attempt}: ${result.error}`);
          return result;
        }

        console.warn(`⚠️ Attempt ${attempt} failed, retrying... (${result.error})`);
